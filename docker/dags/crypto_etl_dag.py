...
def check_new_files(**context):
    """Check for new unprocessed files in MinIO and push them to XCom."""
...
def validate_file_structure(**context):
    """Validate structure of unprocessed files and separate valid from invalid ones."""
    from etl.extract import MinioExtractor
    ...
    for file_path in files_to_process:
        try:
            ...
            if is_valid:
                ...
                logger.info(f"Valid structure: {file_path} ({len(df)} records)")
            else:
                ...
                logger.warning(f"Invalid structure: {file_path} - {errors}")
...
def extract_and_quality_check(**context):
    """Extract data from files and perform quality checks."""
    from etl.extract import MinioExtractor
    from etl.data_quality import DataQualityChecker
    from etl.file_processor import FileProcessingManager
    ...
    for file_info in valid_files:
        ...
        try:
            ...
            if passed:
                ...
                logger.info(f"Quality check passed: {file_path} - {metrics}")
            else:
                ...
                logger.warning(f"Quality check failed: {file_path} - {issues}")
...
def transform_and_load_data(**context):
    """Transform processed data and load into the target database."""
    import pandas as pd
    ...
    for file_info in processed_data:
        ...
        try:
            ...
            logger.info(f"Successfully processed: {file_path} ({len(transformed_df)} records)")
...
def send_success_notification(**context):
    """Send success notification summarizing ETL execution."""
...
def send_failure_notification(**context):
    """Send failure notification when ETL pipeline fails."""
    message = f"""
    ETL Pipeline Failed:
